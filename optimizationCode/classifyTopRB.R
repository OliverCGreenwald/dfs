...
####### DESCRIPTION #######
# In this file we construct a dataset to classify top RBs as "hit" (1) or "not hit" (0).
# Section I: Prepare dataset
# Section II: Machine Learning Algorithms for Classifying Top RBs
...
#     Subsection II: Tuning C by maximizing P(hitting Top RB with reduced set of top RB)
...
#     Subsection II (tuning C by maximizing prob of hitting Top RB):
...
#
# Notes:
#   - logistic regression predicts top QB decently
#   - testing.ind <- sample(x=1:nrow(dataset.all), size=8)
#
# TODO:
...
salary.threshold <- 6500 # define cheap
fpts.threshold <- 30 # define hit
...
####### SECTION I: PREPARE DATAFRAME OF top RB #######
...
for (wk in 4:week.max) { # uncomment for loop if don't want to loop over weeks
  ...
  #---- Print number of Top RB ----#
  print(paste0("Wk ", wk, " Num of Top RB / Num top RB:   ", sum(temp.cheap$Value>0), " / ", nrow(temp.cheap)))
  
  #---- Remove rows with NAs ----#
  temp.dataset <- na.omit(temp.cheap)
  
  #---- Print number of Top RB after removing NAs ----#
  print(paste0("Wk ", wk, " Num of Top RB / Num top RB (NAs Removed):   ", sum(temp.dataset$Value>0), " / ", nrow(temp.dataset)))
...
#---- Print number of Top RB over all weeks (no NAs) ----#
print(paste0("All Weeks, Num of Top RB / Num top RB (NAs Removed):   ", sum(dataset.all$Value>0), " / ", nrow(dataset.all)))
...
testing.ind <- sample(x=1:nrow(dataset.all), size=8) # size=nrow(dataset.all)/5
...
if (model.run==1) {
  ...
  print(paste0("Top RB hit rate w/ classification:   ", confusion.mat[2,2]/(confusion.mat[2,1] + confusion.mat[2,2])))
  print(paste0("Top RB hit rate w/o classification:   ", sum(test.y==1)/length(test.y)))
...
if (model.run==6) {
  ...
  if (model.run.subsection==2) {
    #---- Tuning C by maximizing P(hitting Top RB with reduced set of top RB) ----#
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top RB with reduced set of top RB)', xlab = "Probability")
...
if (model.run==7) {
  ...
  if (model.run.subsection==2) {
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top RB with reduced set of top RB)')
    c.optimal <- tuning.param[which.max(prob.hit.vec)] # optimal tuning parameter C
