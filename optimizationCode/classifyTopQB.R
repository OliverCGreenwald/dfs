...
####### DESCRIPTION #######
# In this file we construct a dataset to classify top QBs as "hit" (1) or "not hit" (0).
# Section I: Prepare dataset
# Section II: Machine Learning Algorithms for Classifying Top QBs
...
#     Subsection II: Tuning C by maximizing P(hitting Top QB with reduced set of top QB)
...
#     Subsection II (tuning C by maximizing prob of hitting Top QB):
...
#
# Notes:
#   - logistic not really good at predicting top QB
#   - testing.ind <- sample(x=1:nrow(dataset.all), size=13)
#
# TODO:
...
fpts.threshold <- 27.5 # define value
...
####### SECTION I: PREPARE DATAFRAME OF top QB #######
...
for (wk in 4:week.max) { # uncomment for loop if don't want to loop over weeks
  ...
  #---- Subset cheap WRs ----#
  temp.cheap <- temp[temp$Salary > salary.threshold & temp$Pos=='QB', c('Player.Name','Likes','Inj','Pos','Salary','Team','Opp','Vegas.Pts','Defense.Pass.Yds.G','Defense.Rush.Yds.G','DvP', "L3.Pass.Att", "S.Pass.Att", "Proj.Pass.Att", "Red.Zone.Pass.Att", "Yards.Per.Pass.Att", "L3.FP", "S.FP", 'Projected.Usage','Floor.FP','Ceil.FP','Proj.FP','Actual.FP')]
  ...
  #---- Print number of Top QB ----#
  print(paste0("Wk ", wk, " Num of Top QB / Num top QB:   ", sum(temp.cheap$Value>0), " / ", nrow(temp.cheap)))
  
  #---- Remove rows with NAs ----#
  temp.dataset <- na.omit(temp.cheap)
  
  #---- Print number of Top QB after removing NAs ----#
  print(paste0("Wk ", wk, " Num of Top QB / Num top QB (NAs Removed):   ", sum(temp.dataset$Value>0), " / ", nrow(temp.dataset)))
...
#---- Print number of Top QB over all weeks (no NAs) ----#
print(paste0("All Weeks, Num of Top QB / Num top QB (NAs Removed):   ", sum(dataset.all$Value>0), " / ", nrow(dataset.all)))
...
testing.ind <- sample(x=1:nrow(dataset.all), size=13) # size=nrow(dataset.all)/5
...
if (model.run==1) {
  ...
  print(paste0("Top QB hit rate w/ classification:   ", confusion.mat[2,2]/(confusion.mat[2,1] + confusion.mat[2,2])))
  print(paste0("Top QB hit rate w/o classification:   ", sum(test.y==1)/length(test.y)))
...
if (model.run==6) {
  ...
  if (model.run.subsection==2) {
    #---- Tuning C by maximizing P(hitting Top QB with reduced set of top QB) ----#
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top QB with reduced set of top QB)', xlab = "Probability")
...
if (model.run==7) {
  ...
  if (model.run.subsection==2) {
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top QB with reduced set of top QB)')
    c.optimal <- tuning.param[which.max(prob.hit.vec)] # optimal tuning parameter C
