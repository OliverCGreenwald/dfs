...
####### DESCRIPTION #######
# In this file we construct a dataset to classify cheap QBs as "value" (1) or "not value" (0).
# Section I: Prepare dataset
# Section II: Machine Learning Algorithms for Classifying Value QBs
...
#
# Notes:
#   - logistic regression predicts value QB surprisingly well
#   - testing.ind <- sample(x=1:nrow(dataset.all), size=20)
#
# TODO:
...
####### SECTION I: PREPARE DATAFRAME OF cheap QB #######
...
for (wk in 4:week.max) { # uncomment for loop if don't want to loop over weeks
  ...
  #---- Print number of Value QB ----#
  print(paste0("Wk ", wk, " Num of Value QB / Num cheap QB:   ", sum(temp.cheap$Value>0), " / ", nrow(temp.cheap)))
  
  #---- Remove rows with NAs ----#
  temp.dataset <- na.omit(temp.cheap)
  
  #---- Print number of Value QB after removing NAs ----#
  print(paste0("Wk ", wk, " Num of Value QB / Num cheap QB (NAs Removed):   ", sum(temp.dataset$Value>0), " / ", nrow(temp.dataset)))
...
#---- Print number of Value QB over all weeks (no NAs) ----#
print(paste0("All Weeks, Num of Value QB / Num cheap QB (NAs Removed):   ", sum(dataset.all$Value>0), " / ", nrow(dataset.all)))
...
testing.ind <- sample(x=1:nrow(dataset.all), size=20) # size=nrow(dataset.all)/5
...
if (model.run==1) {
  ...
  print(paste0("Value QB hit rate w/ classification:   ", confusion.mat[2,2]/(confusion.mat[2,1] + confusion.mat[2,2])))
  print(paste0("Value QB hit rate w/o classification:   ", sum(test.y==1)/length(test.y)))
...
if (model.run==6) {
  ...
  if (model.run.subsection==2) {
    #---- Tuning C by maximizing P(hitting Value QB with reduced set of cheap QB) ----#
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Value QB with reduced set of cheap QB)', xlab = "Probability")
...
if (model.run==7) {
  ...
  if (model.run.subsection==2) {
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Value QB with reduced set of cheap QB)')
    c.optimal <- tuning.param[which.max(prob.hit.vec)] # optimal tuning parameter C
