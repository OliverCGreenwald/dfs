...
####### DESCRIPTION #######
# In this file we construct a dataset to classify value TEs as "value" (1) or "not value" (0).
# Section I: Prepare dataset
# Section II: Machine Learning Algorithms for Classifying Value TEs
...
#     Subsection II: Tuning C by maximizing P(hitting Value TE with reduced set of cheap TE)
...
#     Subsection II (tuning C by maximizing prob of hitting Value TE):
...
#   - testing.ind <- sample(x=1:nrow(dataset.all), size=16)
#   - temp$Proj.FP > 0
#
# TODO:
...
salary.threshold.lo <- 2500 # lower bnd to reduce number of players
fpts.threshold <- 15 # define value
...
####### SECTION I: PREPARE DATAFRAME OF cheap TE #######
...
for (wk in 4:week.max) { # uncomment for loop if don't want to loop over weeks
  ...
  #---- Subset cheap WRs ----#
  temp.cheap <- temp[temp$Proj.FP > 0 & temp$Salary <= salary.threshold & temp$Salary > salary.threshold.lo & temp$Pos=='TE', c('Player.Name','Likes','Inj','Pos','Salary','Team','Opp','Vegas.Pts','Defense.Pass.Yds.G','Defense.Rush.Yds.G','DvP','L3.Rush.Att','S.Rush.Att','Proj.Rush.Att','Red.Zone.Rush.Att','Yards.Per.Rush.Att', "L3.Targets", "S.Targets", "Proj.Targets", "Red.Zone.Targets", "Yards.Per.Target", "L3.FP", "S.FP", 'Projected.Usage','Floor.FP','Ceil.FP','Proj.FP','Actual.FP')]
  ...
  #---- Print number of Value TE ----#
  print(paste0("Wk ", wk, " Num of Value TE / Num cheap TE:   ", sum(temp.cheap$Value>0), " / ", nrow(temp.cheap)))
  
  #---- Remove rows with NAs ----#
  temp.dataset <- na.omit(temp.cheap)
  
  #---- Print number of Value TE after removing NAs ----#
  print(paste0("Wk ", wk, " Num of Value TE / Num cheap TE (NAs Removed):   ", sum(temp.dataset$Value>0), " / ", nrow(temp.dataset)))
...
#---- Print number of Value TE over all weeks (no NAs) ----#
print(paste0("All Weeks, Num of Value TE / Num cheap TE (NAs Removed):   ", sum(dataset.all$Value>0), " / ", nrow(dataset.all)))
...
testing.ind <- sample(x=1:nrow(dataset.all), size=16) # size=nrow(dataset.all)/5
...
if (model.run==1) {
  ...
  print(paste0("Value TE hit rate w/ classification:   ", confusion.mat[2,2]/(confusion.mat[2,1] + confusion.mat[2,2])))
  print(paste0("Value TE hit rate w/o classification:   ", sum(test.y==1)/length(test.y)))
...
if (model.run==6) {
  ...
  if (model.run.subsection==2) {
    #---- Tuning C by maximizing P(hitting Value TE with reduced set of cheap TE) ----#
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Value TE with reduced set of cheap TE)', xlab = "Probability")
...
if (model.run==7) {
  ...
  if (model.run.subsection==2) {
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Value TE with reduced set of cheap TE)')
    c.optimal <- tuning.param[which.max(prob.hit.vec)] # optimal tuning parameter C
