...
####### DESCRIPTION #######
# In this file we construct a dataset to classify top WRs as "hit" (1) or "not hit" (0).
...
#     Subsection II: Tuning C by maximizing P(hitting Top WR with reduced set of cheap WR)
...
#     Subsection II (tuning C by maximizing prob of hitting Top WR):
...
#
# Notes:
#   - testing.ind <- sample(x=1:nrow(dataset.all), size=15)
#
# TODO:
...
salary.threshold <- 6500 # define top
fpts.threshold <- 18.5 # 18.5 # define hit
...
for (wk in 4:week.max) { # uncomment for loop if don't want to loop over weeks
  ...
  #---- Print number of Top WR ----#
  print(paste0("Wk ", wk, " Num of Top WR / Num Cheap WR:   ", sum(temp.cheap$Value>0), " / ", nrow(temp.cheap)))
  
  #---- Remove rows with NAs ----#
  temp.dataset <- na.omit(temp.cheap)
  
  #---- Print number of Top WR after removing NAs ----#
  print(paste0("Wk ", wk, " Num of Top WR / Num Cheap WR (NAs Removed):   ", sum(temp.dataset$Value>0), " / ", nrow(temp.dataset)))
...
#---- Print number of Top WR over all weeks (no NAs) ----#
print(paste0("All Weeks, Num of Top WR / Num Cheap WR (NAs Removed):   ", sum(dataset.all$Value>0), " / ", nrow(dataset.all)))
...
testing.ind <- sample(x=1:nrow(dataset.all), size=15) # size=nrow(dataset.all)/5
...
if (model.run==1) {
  ...
  print(paste0("Top WR hit rate w/ classification:   ", confusion.mat[2,2]/(confusion.mat[2,1] + confusion.mat[2,2])))
  print(paste0("Top WR hit rate w/o classification:   ", sum(test.y==1)/length(test.y)))
...
if (model.run==6) {
  ...
  if (model.run.subsection==2) {
    #---- Tuning C by maximizing P(hitting Top WR with reduced set of cheap WR) ----#
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top WR with reduced set of cheap WR)', xlab = "Probability")
...
if (model.run==7) {
  ...
  if (model.run.subsection==2) {
    ...
    plot(log(tuning.param), prob.hit.vec, type='l', main='P(hitting Top WR with reduced set of cheap WR)')
    c.optimal <- tuning.param[which.max(prob.hit.vec)] # optimal tuning parameter C
